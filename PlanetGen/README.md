...
- Cross-face LOD balancing (`bEnableCrossFaceLODBalancing`) — off by default. The
  ...
  geometrically correct on all 24 edges — see `Docs/03_SeamValidation.md`. Still flagged
  off by default pending an in-PIE flight-test of the actual split-cascade behavior (the
  fix proves the coordinate transform is correct, not that the whole balancing pass is
  bug-free end to end). Fixed in both packages.
