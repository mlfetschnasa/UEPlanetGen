	...
	ENoiseDebugView DebugView)
{
	...
	// Triangle-averaged normals: used for slope detection (vertex color Alpha) AND kept
	// as the mesh normals -- the material blends VertexNormalWS with the analytical
	// sphere normal by camera distance for seamless-at-distance lighting.
	...
	// Compute slope from triangle normals.
	// Slope = deviation of triangle normal from radial direction (0=flat, 1=vertical cliff).
	// Stored in vertex color Alpha -- Snow is derived in material as (1-R-G-B) so Alpha is free.
	...
	// NOTE: triangle-averaged normals are kept in Out.Normals (NOT replaced with analytical
	// sphere normals). The material blends between Vertex Normal WS (triangle normals, surface
	// detail up close) and the analytical normal (computed in material, seamless at distance).
	// Replacing normals here with radial direction would make VertexNormalWS identical to the
	// analytical normal, defeating the close-up surface detail blend.
	...
	const TArray<FStaticPOI>& POIs)
{
	...
	auto ProcessType = [&](const FFoliageTypeSettings& T, uint32 TypeSalt, bool bIsGrassChannel, TArray<FTransform>& OutTransforms)
	{
		...
		for (int32 T_i = 0; T_i < TriCount; ++T_i)
		{
			...
			const double AreaM2 = AreaCm2 / 10000.0;
...
{
	if (bEnabled == bFoliageEnabled) return;
